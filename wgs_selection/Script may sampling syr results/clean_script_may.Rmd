...
read_tables <- function(region) { # table and y: region in format the following format Region_O for orchard, or Region_W for woodland (e.g. Kent_O)
  ...
  table <- table[c("Region", "Labelling", "Isolate", "Plate", "Column", "Row", "Total ccupancy (0: no colony; 1:10%; 5:50%; 9:90%,10: fully covered)",	"Number of coloy types",	"Transluscent",	"Fluorecent (green)",	"White", "Yellow",	"Others", "syr", "non-specific binding",	"syr PCR notes", "PCR results", "Notes on PCR")] # select only those columns of interest "Region", "Labelling", "Isolate", "syr", "PCR results" or "Region", "Labelling", "Isolate", "Plate", "Column", "Row", "Total ccupancy (0: no colony; 1:10%; 5:50%; 9:90%,10: fully covered)",	"Number of coloy types",	"Transluscent",	"Fluorecent (green)",	"White",	"Yellow",	"Others", "syr", "non-specific binding",	"syr PCR notes", "PCR results", "Notes on PCR"
...
cultivar_tree <- function(table, type, place){
  ...
  if(type=="tree"){
    ...
    table$`syr` <- ifelse (table$`syr` %in% c("1", "0.5", "0.5?"), 1, 0) # change "1" to "1", "0.5", "0.5?"
    table1 <- as.data.frame(dcast(table, `Full place w/ tree`~Tissue, value.var = "syr",sum))
...
cultivar_tree_hrc <- function(table, syr, place, syr_type){ #change to ps_summary_table? Change place to site?
  ...
  if(syr=="1"){
    ...
    table1 <-  table1[,-1:-3]
    table1
    table2 <- as.data.frame(dcast(table, `Full place w/ tree`~Tissue, value.var = "syr", sum))
    ...
    table$`Ps presence` <- ifelse (table$`Ps presence` == "Confirmed positive possibly pathogenic", 1, 0)
      if(syr_type=="a"){
        table$`syr` <- ifelse (table$syr %in% c("1", "0.5", "0.5?"), 0, 1)
      } else if(syr_type=="b"){
        table$`syr` <- ifelse (table$syr %in% c("1"), 0, 1)
      }
    table1 <- as.data.frame(dcast(table, `Full place w/ tree`~Tissue, value.var = "Ps presence", sum))
...
# ---------------------------------------------------------------
#  Table with number of postive strains at tree and tissue level
# ---------------------------------------------------------------
...
saveWorkbook(wb, file = "Summary of 21May PCR tree and tissue levels syr+ hrc_trial.xlsx", overwrite = TRUE)
...
# ---------------------------------------------------------------------------------------------------------
#  Table of useful trees among the syr+ve ones, how many are hrcC+ve; (2)the numbers of hrcC+ve but syr-ve
# ---------------------------------------------------------------------------------------------------------
...
Ps_table_full_ <- Ps_table_full[Ps_table_full$syr %in% c("1", "0.5", "0.5?"),] # change "1" to "1", "0.5", "0.5?"
...
Ps_table_full_L$syr[Ps_table_full_L$syr == "0.5?"] <- 0.1
...
Ps_table_full_L_ <- Ps_table_full_L %>%
  ...
  #sample_n(nrow(.)) %>%
  arrange(desc(syr), desc(`Ps presence`)) %>% #add , desc(`Ps presence`)
  slice(1:1) %>%
  ungroup() %>%
  sample_n(nrow(.)) %>%
  group_by(`Full place`) %>%
...
Ps_table_full_S$syr[Ps_table_full_S$syr == "0.5?"] <- 0.1
...
Ps_table_full_S_W <- Ps_table_full_S__[Ps_table_full_S_$Place=="W",]

Ps_table_full_S_O <- Ps_table_full_S__[Ps_table_full_S_$Place=="O",]
...
saveWorkbook(wb, file = "WGS_selection_shoots_V2.xlsx", overwrite = TRUE)
...
saveWorkbook(wb, file = "WGS_count_shoots_V2.xlsx", overwrite = TRUE)
...
Ps_table_full_ <- Ps_table_full[Ps_table_full$syr %in% c("1", "0.5", "0.5?"),] # change "1", "0.5", "0.5?" to "1" when needed. Selection of Psy+ results
Ps_table_full_$`Ps presence` <- ifelse (Ps_table_full_$`Ps presence` == "Confirmed positive possibly pathogenic", 1, 0)
Ps_table_full_L <- Ps_table_full_[Ps_table_full_$Tissue=="L",] 
Ps_table_full_L$syr[Ps_table_full_L$syr == "0.5?"] <- 0.1
...
#saveWorkbook(wb, file = "strain_WGS_list_syr_leaves_hrcC+_V2.xlsx", overwrite = TRUE)
saveWorkbook(wb, file = "strain_WGS_list_syr_leaves_batch2_strains_V1(same as v4_copy).xlsx", overwrite = TRUE)
...
saveWorkbook(wb, file = "WGS_strains_count_leaves_batch2_strains_V1(same as v4_copy).xlsx", overwrite = TRUE)
...
Ps_table_full_ <- Ps_table_full[Ps_table_full$syr %in% c("1", "0.5", "0.5?"),] # change "1" to "1", "0.5", "0.5?" when needed. Selection of Psy+ results
Ps_table_full_$`Ps presence` <- ifelse (Ps_table_full_$`Ps presence` == "Confirmed positive possibly pathogenic", 1, 0)
Ps_table_full_L <- Ps_table_full_[Ps_table_full_$Tissue=="L",] 
Ps_table_full_L$syr[Ps_table_full_L$syr == "0.5?"] <- 0.1
